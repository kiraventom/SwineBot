...
namespace SwineBot.Utils;

public class GraphBuilder(ILogger<GraphBuilder> logger, UserContext context)
{
    public async Task<byte[]> DrawGraph(Func<UserContext, IQueryable<Swine>> selector)
    {
        var swines = await selector.Invoke(context).ToListAsync();
